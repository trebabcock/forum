<template>
  <div class="flex flex-col space-y-4 h-full">
    ...
    >
      <div
        class="flex flex-col w-64 space-y-4 items-center justify-center font-bold"
      >
        <span class="text-2xl">{{ board.name }}</span>
        <div
          class="w-36 h-10 cursor-pointer rounded-lg shadow flex items-center justify-center hover:bg-slate-700 bg-slate-900"
          @click="toCreate"
        >
    ...
    <div
      class="w-full h-full flex flex-col flex-grow py-4 px-4 bg-slate-800 rounded-lg shadow-lg space-y-2"
    >
      <post-item
        v-for="post in posts"
        :key="post.id"
