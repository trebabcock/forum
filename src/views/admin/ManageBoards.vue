<template>
  <div class="flex justify-center items-center w-full h-full">
    <div class="flex flex-col p-6 space-y-4 w-96 items-center">
      <input
        class="px-4 w-full py-2 bg-slate-800 rounded-lg outline-none hover:bg-slate-600 focus:bg-slate-600 shadow-lg"
        type="text"
        v-model="board_name"
        placeholder="Board Name"
      />
      <textarea
        class="p-4 w-full bg-slate-800 hover:bg-slate-600 rounded-lg outline-none focus:bg-slate-600 shadow-lg h-64"
        placeholder="Board Description"
        v-model="description"
      ></textarea>
      <div class="flex">
        <div
          class="rounded-lg h-10 py-2 px-4 w-30 bg-slate-800 hover:bg-slate-600 cursor-pointer block shadow-lg"
          @click="createBoard()"
        >
          <span class="text-slate-50 block">Create Board</span>
        </div>
      </div>
    </div>
    <div class="flex flex-col p-6 space-y-4 w-96 items-center">
      <div class="w-full">
        <Listbox v-model="selected_board">
          <div class="relative mt-1">
            <ListboxButton
              class="relative w-full py-2 pl-4 pr-10 text-left bg-slate-800 text-slate-50 hover:bg-slate-600 rounded-lg shadow-lg cursor-default focus:outline-none sm:text-sm"
            >
            ...
            >
              <ListboxOptions
                class="absolute w-full p-1 mt-1 overflow-auto text-base bg-slate-900 text-slate-50 rounded-lg shadow-lg max-h-60 ring-opacity-5 focus:outline-none sm:text-sm"
              >
                <ListboxOption
                ...
                >
                  ...
                  >
                    ...
                    <span
                      v-if="selected"
                      class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-50"
                    >
      ...
      <div class="flex space-x-16">
        <div
          class="rounded-lg h-10 px-4 py-1 flex items-center justify-center bg-slate-800 hover:bg-slate-600 cursor-pointer shadow-lg w-24"
          @click="updateBoard"
        >
          <span class="text-slate-50 font-bold text-lg">Save</span>
        </div>
        ...
        <div
          ...
          @click="openModal"
        >
          <span class="text-slate-50 font-bold text-lg">Delete</span>
        </div>
      </div>
    </div>
      ...
      <Dialog as="div" @close="closeModal">
        <div class="fixed inset-0 z-10 overflow-y-auto">
          <div class="min-h-screen px-4 text-center">
            ...
            >
              <div
                class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform text-slate-50 bg-slate-900 shadow-xl rounded-lg"
              >
